...
import { AccessSubProducts7 } from './AccessSubProducts7';

const AccessProducts7 = () => {
	const { productid } = useParams();
	const [isOpen, setIsOpen] = useState(false);
	const { kids, currency } = useContext(ShopContext);
	const [productData, setProductData] = useState(false);

	const fetchData = () => {
		const product = kids.find((item) => item._id == productid);
		if (product) {
	...
	return productData ? (
		<>
			<div className='pt-10 transition-opacity ease-in duration-500 container mx-auto px-4 sm:px-6 lg:px-8'>
				...
				<AccessSubProducts7 category={productData.category} />
			</div>
...
export default AccessProducts7;
