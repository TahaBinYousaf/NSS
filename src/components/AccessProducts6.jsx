...
import { AccessSubProducts6 } from "./AccessSubProducts6";
import { FaApple } from "react-icons/fa6";

const AccessProducts6 = () => {
  const { productid } = useParams();
  const [isOpen, setIsOpen] = useState(false);
  const { animals, currency } = useContext(ShopContext);
  const [productData, setProductData] = useState(false);

  const fetchData = () => {
    const product = animals.find(item => item._id == productid);
    if (product) {
  ...
  return productData ? (
    <>
      <div className="pt-10 transition-opacity ease-in duration-500 container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col-reverse lg:flex-row w-full justify-between items-start">
          <div className="w-full sm:w-1/2 border border-gray-300 rounded-xl py-6 px-4 sm:px-8 my-8">
            <h1 className="text-4xl sm:text-5xl font-bold">
              {currency}
              {productData.price}
            </h1>
            <h1 className="py-4 text-2xl sm:text-3xl font-bold">{productData.description}</h1>
          ...
          <div className="w-full h-[300px] lg:w-1/3 border border-gray-300 rounded-xl p-4 sm:p-6 flex flex-col gap-4">
            ...
            <div className="flex gap-4 items-center">
              <img src={Image5} className="w-16 h-16 rounded-full border-2 border-gray-300 object-cover" alt="Seller" />
        ...
        <div className="w-full sm:w-1/2 border border-gray-300 rounded-xl py-6 px-4 sm:px-8 my-8">
          ...
          <p style={{ whiteSpace: "pre-line" }}>{productData.name}</p>
        </div>

        <AccessSubProducts6 category={productData.category} />
      </div>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-99 backdrop-blur-md flex items-center justify-center">
          <div className="bg-white rounded-lg p-6 w-full max-w-[400px] shadow-lg relative mx-4">
            ...
            <h2 className="text-center text-xl font-semibold text-gray-800">Login into your NSS account</h2>
...
export default AccessProducts6;
