...
import { AccessSubProducts5 } from './AccessSubProducts5';

const AccessProducts5 = () => {
	const { productid } = useParams();
	const [isOpen, setIsOpen] = useState(false);
	const { bikes, currency } = useContext(ShopContext);
	const [productData, setProductData] = useState(false);

	const fetchData = () => {
		const product = bikes.find((item) => item._id == productid);
		if (product) {
			...
			<div className='pt-10 transition-opacity ease-in duration-500 container mx-auto px-4 sm:px-6 lg:px-8'>
				...
				<div className='w-full sm:w-1/2 border border-gray-300 rounded-xl py-6 px-4 sm:px-8 my-8'>
					...
					<div className='flex  items-center justify-between gap-4'>
						<p>Resource Type</p>
						<p className='font-bold'>{productData.resource_Type}</p>
					</div>
				</div>
				...
				<AccessSubProducts5 category={productData.category} />
			</div>
...
export default AccessProducts5;
